"""
5backtest_baseline_logreg.py
============================
Backtest baseline untuk skripsi — Logistic Regression.

Baca prediksi dari 4train_baseline_logreg.py → simulasi eksekusi → equity curve.
Semua parameter IDENTIK dengan 5backtest.py agar perbandingan adil.

Output:
  data/baseline_backtest_trades.csv  ← detail setiap trade
  data/baseline_backtest_report.csv  ← ringkasan per tahun
  data/baseline_equity_curve.csv     ← equity curve harian (untuk chart)
"""
...
# ══════════════════════════════════════════════════════════════
# PARAMETER TERKUNCI  (identik persis dengan 5backtest.py)
# ══════════════════════════════════════════════════════════════
PRED_PATH       = "data/baseline_predictions.csv"   # ← satu-satunya beda
INITIAL_BAL     = 1000.0
...
# ══════════════════════════════════════════════════════════════
print("=" * 65)
print(" BACKTEST BASELINE — Rolling Yearly | Logistic Regression")
print("=" * 65)

# ── Load prediksi ─────────────────────────────────────────────
if not os.path.exists(PRED_PATH):
    print(f"File {PRED_PATH} tidak ditemukan.")
    print("Jalankan dulu: python 4train_baseline_logreg.py")
    exit()
...
# ── Report ────────────────────────────────────────────────────
print("\n" + "=" * 65)
print(f"{'  OVERALL PERFORMANCE REPORT (BASELINE — LR)  ':=^65}")
...
res_out.to_csv("data/baseline_backtest_trades.csv", index=False)
eq.to_csv("data/baseline_equity_curve.csv", index=False)
...
pd.DataFrame(report_rows).to_csv("data/baseline_backtest_report.csv", index=False)
...
print("\nFile output:")
print("  data/baseline_backtest_trades.csv")
print("  data/baseline_backtest_report.csv")
print("  data/baseline_equity_curve.csv")
print("\nJalankan berikutnya: python 99compareModels.py")